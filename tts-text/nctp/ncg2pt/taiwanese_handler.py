...
from nctp.ncg2pc.chinese_handler import PINYIN_DICT, is_zh, ChineseProcessor
# p = Pinyin()
...
class TaiwaneseProcessor(ChineseProcessor):
    def __init__(self,):
        super().__init__()
        # artifactory
        self.pinyin_dict.update(**TWN_PINYIN_DICT)
    
    def get_pinyin_parser(self):
        self.init_g2pw()
        pinyin = self.g2pw.lazy_pinyin
        return pinyin
    
    def init_g2pw(self):
        print("G2P module initializing... This function runs only once when TTS is run for the first time.")
        NCTTS_TM = os.environ.get("NCTTS_TM")
        try:
            model_dir = f"{NCTTS_TM}/taiwanese_processor/G2PWModel/"
            model_source = f"{NCTTS_TM}/taiwanese_processor/G2PWModel/bert-base-chinese/"
            self.g2pw = G2PWPinyin(model_dir=model_dir,
...
if __name__ == "__main__":
    ...
    # tp = TextProcessor('tai', 'baker', use_g2p=True)
    # print(tp._symbols)
    # print(tp.text_to_sequence("妈妈当时表示，儿子开心得像花儿一样。", inference=True))
    ...
    # tp = TaiwaneseProcessor()
    # text = "你#1可以#1先#1出门#3,我不#1知道#1穿#1什么#1出去#4.|ni3 ke6 yi3 xian1 cu1 men2 wo3 bu4 zao1 _ cuan1 sen3 me5 cu1 qu4"
    # han, phn = text.split("|")
    # print(tp.get_phoneme_from_char_and_pinyin(han, phn))

    NCTTS_TM = os.environ.get("NCTTS_TM")
    model_dir = f"/SGV/users/mkyu/proj/NCTTSs/NCTTS-bitbang/tts-engine/nctts.tm/taiwanese_processor/G2PWModel/"
    model_source = f"/SGV/users/mkyu/proj/NCTTSs/NCTTS-bitbang/tts-engine/nctts.tm/taiwanese_processor/G2PWModel/bert-base-chinese"
    g2pw = G2PWPinyin(model_dir=model_dir,
                    model_source=model_source,
                    v_to_u=False, neutral_tone_with_five=True)
    
    print(g2pw.lazy_pinyin("ABC要一杯朗姆酒嗎？", style=Style.TONE3, errors="ignore"))